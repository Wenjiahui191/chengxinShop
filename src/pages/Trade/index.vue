  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="(addr, index) in UserAddress"
        :key="addr.id"
      >
        <span class="username" :class="{ selected: addr.isDefault == 1 }">{{
          addr.consignee
        }}</span>
        <p @click="chooseAddr(addr, UserAddress)">
    ...
    <div class="sub clearFix">
      <a href="#" class="subBtn" @click="submitPay">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      // total为不包含运费和返现的总价 total为包含所有费用的结算价
      return total;
    },
    // 需要向服务器提交的数据
    submitData() {
  ...
  methods: {
    chooseAddr(addr, addrInfo) {
      addrInfo.forEach((item) => (item.isDefault = 0));
      addr.isDefault = 1;
    },
    // 提交订单信息向服务器发送数据
    async submitPay() {
      let result = await this.$API.reqSubmitPay(
